"""
Parts of this code were taken from the Open Catalyst repository https://github.com/Open-Catalyst-Project/ocp, under an MIT License
"""
...
def load_toy_data():
    x1 = torch.tensor([3, 2, 2, 1]).long().unsqueeze(-1)
    edge_index1 = torch.tensor([[1, 0, 3, 1, 0],
                                [0, 1, 1, 2, 3]])

    x2 = torch.tensor([2, 1, 2]).long().unsqueeze(-1)

    edge_index2 = torch.tensor([[1, 2, 1],
                                [0, 0, 2]])

    data1 = pyg.data.Data(x1, edge_index1)
    data2 = pyg.data.Data(x2, edge_index2)

    return data1, data2

def get_unit_cell(device):
    max_rep = [2, 2, 2]
    ...
    return torch.cartesian_prod(*cells_per_dim)
...
def radius_graph_pbc(atom_pos, cell, natoms, radius, max_num_neighbors_threshold):
    ...
    unit_cell = get_unit_cell(device)
...
def generate_otf_graph(data, radius, max_neighbors, pbc):
    if pbc:
        edge_index, cell_offsets, neighbors = radius_graph_pbc(data.pos, data.cell, data.natoms, radius, max_neighbors)
        out = get_pbc_distances(data.pos, edge_index, data.cell, cell_offsets, neighbors, return_offsets=False,
